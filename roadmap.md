...
- [ ] **HIGH — CONFIRMED across four doc sites: the annotations/per-device-progress claim ("on each detail page", "already rendered in the Codex", metainfo 1.3.0) describes UI that was never built (wrappers tested, zero UI call sites).** Recommended: wire the Codex annotations section + the cover-progress "maybe" (per-book reads dodge the N+1); alternatively reword all four sites. *(gated: wire-vs-reword was asked 2026-09-14; the four doc sites stay untouched pending the answer.)*
...
- [ ] MED — metainfo 1.3.0 release note carries the never-built annotations claim (Flathub-facing; annotate the correction, never silently rewrite shipped notes); the user-site install is a REAL non-editable 1.5.0 copy, so the PATH console scripts (hermitage, hermitage-verify) run four-releases-old code — force-reinstall editable. *(the 1.3.0 annotation folds into the wire-vs-reword answer; the user-site 1.5.0 reinstall is Brandon's environment — both stay open.)*
