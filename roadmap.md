...
- [ ] **HIGH — CONFIRMED: _find_format_file recurses infinitely when no format file resolves: the for-loop rebinds the fmt parameter, the retry gate tests the leaked last element, and every Read click on a fileless book burns ~1000 SQL-re-running stack levels into RecursionError (codex.py:148-162; zero test coverage).** Save requested=fmt before the loop, gate on it, delete the dead second fallback loop, add the test.
- [ ] **HIGH — NEW: the Flatpak pin cannot build under the runtime it targets: yml:153 pins cquarry 132aa2c (requires-python >=3.14, PEP 758 bare groups = SyntaxError ≤3.13) while the GNOME 50 runtime ships Python 3.13.15 and pip 26 enforces Requires-Python at install; no 3.13-compatible cquarry tag exists (v1.9.0+ all 3.14).** The "pin COMPLIANT" audit-record claims and the metainfo 1.8.3 entry are falsified. Decide: 3.14-based runtime vs upstream cquarry 3.13-compat branch; state the effective floor (3.14) in README/pyproject/spec/CLAUDE.
- [ ] **HIGH — CONFIRMED across four doc sites: the annotations/per-device-progress claim ("on each detail page", "already rendered in the Codex", metainfo 1.3.0) describes UI that was never built (wrappers tested, zero UI call sites).** Recommended: wire the Codex annotations section + the cover-progress "maybe" (per-book reads dodge the N+1); alternatively reword all four sites.
- [ ] MED — UI responsiveness cluster: library load runs on the main thread via idle_add and catches only FileNotFoundError (corrupt/locked DB strands the spinner; the follow-up calls sit outside the try); cover_path evaluated 3× per book incl. per grid bind (each a SQL SELECT); five ThreadPools never shut down (quit drains the multi-thousand-job warm queue); texture LRU is ~1.6 GB at 2x (entry-capped 512 × ~3.1 MB textures); export runs bulk reads on the main thread.
- [ ] MED — Portal scheme handler applies scheme==1 while _query_dark returns scheme!=2 (a live flip to "no preference" goes light; the same value at startup stays dark; the comment records that ==1 "silently broke"); enum-pill provider load_from_string replaces its own CSS per color class (2+ colored values → earlier pills lose their tint).
- [ ] MED — Small correctness: color cache keyed by book_id alone (cover changes keep the stale glow color); DecompressionBombError escapes all three Pillow handlers silently; get_all_comments swallows everything so export can silently write comment:null library-wide (contradicts the "synopses never export as absent" contract); book.cover_path ValueError mid-bind leaves recycled cells with mismatched title/cover.
- [ ] MED — Docs truth: CLAUDE.md:29 + spec.md:14 cquarry floor stale (≥1.8 → ≥1.18); README:152 overstates hermitage-verify (catalog presence, not disk; png-only covers count missing by design); spec.md:35 "No settings menu" contradicts the shipped Preferences window; spec.md:37-38 personal path + garbled all-caps warning; README:173 calls database.py a parser (thin cquarry wrapper since v1.2.0); README:184 attributes mode=ro to Hermitage (cquarry is the enforcement layer — credit it); README never mentions the shipped Flatpak.
- [ ] MED — Flathub gate pair: add the AppStream <screenshots> block (the TODO's precondition met since v1.0.0; capture Codex/genre/series/Insights at 1920x1080) and switch the manifest source from type:dir to type:git + tag (v1.8.3 exists). Both ride Brandon's Flathub onboarding.
- [ ] MED — metainfo 1.3.0 release note carries the never-built annotations claim (Flathub-facing; annotate the correction, never silently rewrite shipped notes); the user-site install is a REAL non-editable 1.5.0 copy, so the PATH console scripts (hermitage, hermitage-verify) run four-releases-old code — force-reinstall editable.
- [ ] LOW — Cache/resource tail: disk caches grow forever (mtime-keyed superseded entries; size-aware startup sweep); png-only covers show the placeholder (documented gap, inconsistent with Insights' counting); wizard do_activate ordering fragility; per-bind provider churn; refresh_library has zero call sites (wire as the Reload Library rider); LOAD_TRUNCATED_IMAGES re-set per call on a worker; type-ahead rebuilds the title list per keystroke; redundant (GLib.Error, Exception) groups.
- [ ] LOW — Comment truth: insights.py "no identifiers predicate upstream" stale since 1.8.2; get_comment_for "short-lived CalibreDB" docstring is false (app-lifetime singleton); the spinner comment describes motion that freezes; get_cquarry_db lacks its two invariant comments (read-only provenance; main-thread-only connection, workers open their own); series.py "(gaps)" vs code's "(incomplete)"; custom-field comment vs annotation mismatch; \u2014 escapes vs literals; avg_rating_x10 naming; year-101 sentinel comment duplication; LRU comment to be rewritten in bytes. REFUTED lead: thumbnailer's "(thread-safe)" docstring is CORRECT per the GTK 4.22 GIR (gdk_texture_new_from_filename is threadsafe) — do not change it.
- [ ] LOW — Housekeeping: .gitignore lacks .claude/ (global-ignore-masked); delete the empty result_images/ skeleton and the retired-client .claude/ dir; ~174 MB of all-ignored build residue (.flatpak-builder 154M, build-flatpak, build-dir, build/, egg-info with its stale 1.5.0 PKG-INFO) reclaimable opportunistically; the stock .gitignore template carries dead Django/Flask sections; CI lint never covers tests/ (one-line widen); v1.0.0 release title drops its milestone suffix.
- [ ] Feature candidates logged (FINAL-REPORT L4, ranked): content: full-text search (cquarry's search_book_text ships unused in the pin; 7,400+ books; off-thread + cancellation; decide grammar-compose vs standalone pre-filter); the S-rider bundle (Reload Library, clickable Insights audit rows, avg-rating genre pills, enum label_index ordering); author browser (the primary fiction dimension has no browse page); export-current-view (~20 lines, export_books verbatim); Recently Added shelf row; cover-size preference; cache-management row. Declined: multi-library switching, OPDS (Carrel serves it), the grid-top recency shelf (closed call).
