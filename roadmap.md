...
- [x] **Search-expression parity:** cross-check the search grammar against cquarry's `_parse_or` / `_match_tags` / `_match_authors` semantics (especially `tags:Foo` matching `Foo.*` as a hierarchy prefix, which Hermitage's retired local parser did not do). Closed by the v1.2.0 port itself: Hermitage evaluates every query through cquarry, so the same query returns the same set by construction.
...
- [x] **Build a test suite.** Shipped in v0.16.0: 63 unittest tests under `tests/` in the CalibreQuarry style (stdlib unittest, temp sqlite fixtures). Covers the search grammar, the database layer against a hand-built `metadata.db` fixture, config, export, reading history, and the genre/series/insights aggregation builders. (The locked-DB snapshot fallback named here when written moved to cquarry at the v1.2.0 port, along with its test coverage; Hermitage's own suite has never carried a lock test.) Run with `python -m unittest discover -s tests`.
...
## Phase 16: cquarry Integration (re-scoped 2026-09-02; originally the second "Phase 4")
...
### Final audit 2026-09-13 (THE FINAL AUDIT: NEW findings, one line each; full detail in audit-final/Hermitage/FINAL-REPORT.md)
- [x] **HIGH — CONFIRMED: _find_format_file recurses infinitely when no format file resolves: the for-loop rebinds the fmt parameter, the retry gate tests the leaked last element, and every Read click on a fileless book burns ~1000 SQL-re-running stack levels into RecursionError (codex.py:148-162; zero test coverage).** Save requested=fmt before the loop, gate on it, delete the dead second fallback loop, add the test. *(v1.8.4: requested saved before the loop, the dead double fallback deleted, four regression tests including two that RecursionError against the old code.)*
- [ ] **HIGH — NEW: the Flatpak pin cannot build under the runtime it targets: yml:153 pins cquarry 132aa2c (requires-python >=3.14, PEP 758 bare groups = SyntaxError ≤3.13) while the GNOME 50 runtime ships Python 3.13.15 and pip 26 enforces Requires-Python at install; no 3.13-compatible cquarry tag exists (v1.9.0+ all 3.14).** The "pin COMPLIANT" audit-record claims and the metainfo 1.8.3 entry are falsified. Decide: 3.14-based runtime vs upstream cquarry 3.13-compat branch; state the effective floor (3.14) in README/pyproject/spec/CLAUDE. *(v1.8.4 executed the ungated half: pyproject declares >=3.14 with the reasoning, README/spec/CLAUDE state the effective floor, the metainfo 1.8.3 entry is annotated, and a ruff target-version py313 pin now guards the parenthesized-except discipline. The runtime-vs-compat-branch decision stays with Brandon.)*
- [x] **HIGH — CONFIRMED across four doc sites: the annotations/per-device-progress claim ("on each detail page", "already rendered in the Codex", metainfo 1.3.0) describes UI that was never built (wrappers tested, zero UI call sites).** Recommended: wire the Codex annotations section + the cover-progress "maybe" (per-book reads dodge the N+1); alternatively reword all four sites. *(v1.8.5: Brandon ruled wire on 2026-09-14. A Reading Progress section (per-device latest positions, newest first, via the new get_reading_progress_by_device wrapper) and an Annotations section (highlights quoted, bookmarks by title, notes) render in the Codex, read per book on activation and hidden when a book has no rows. The cover-progress bar stays a recorded maybe. Verified on the live session.)*
- [x] MED — UI responsiveness cluster: library load runs on the main thread via idle_add and catches only FileNotFoundError (corrupt/locked DB strands the spinner; the follow-up calls sit outside the try); cover_path evaluated 3× per book incl. per grid bind (each a SQL SELECT); five ThreadPools never shut down (quit drains the multi-thousand-job warm queue); texture LRU is ~1.6 GB at 2x (entry-capped 512 × ~3.1 MB textures); export runs bulk reads on the main thread. *(v1.8.4: worker-thread load on its own CalibreDB with broadened error handling and a destroy guard; cover paths memoized per Book so binds stop re-running the SELECT; five executors cancelled on quit; texture LRU byte-bounded at 512 MiB. Export-on-main-thread carved out as its own Phase 17 box.)*
...
- [x] Feature candidates logged (FINAL-REPORT L4, ranked): content: full-text search (cquarry's search_book_text ships unused in the pin; 7,400+ books; off-thread + cancellation; decide grammar-compose vs standalone pre-filter); the S-rider bundle (Reload Library, clickable Insights audit rows, avg-rating genre pills, enum label_index ordering); author browser (the primary fiction dimension has no browse page); export-current-view (~20 lines, export_books verbatim); Recently Added shelf row; cover-size preference; cache-management row. Declined: multi-library switching, OPDS (Carrel serves it), the grid-top recency shelf (closed call). *(slotted: Phase 17 carries the L4 order plus two new-work boxes noticed while executing.)*

**CONFIRMED-prior (final-audit verification):** the recursion bug, the enum-pill/portal/load/executor/LRU cluster, the annotations claim, both floor-stale sites, the screenshots TODO, the spec personal block, the dead surfaces, the .gitignore gap, the build residue. WITHDRAWN with evidence: the Wave-16 "Insights fallback unprotected" lead (the 1.8.1 guard is present and test-pinned). REFUTED: the texture thread-contract lead. SUPERSEDED (verified live): the entire decision-60 GitHub set. Record corrections: ab55870 IS pushed; the "pip editable dogfood" note is wrong (user-site 1.5.0); the Flatpak-pin compliance claims are falsified by live measurement. Slop-reader verdict: human almost end to end; the spec opener is the one model-pitching passage; the em-dash layer is mechanical (README 3+8 ASCII, CLAUDE 7, roadmap block 5 spaced hyphens).

## Phase 17: Feature candidates (slotted from the final audit, L4 order preserved)

Slotted, not scheduled: each becomes real work only on its own green
...
- [ ] **`content:` full-text search** — cquarry's `search_book_text()`
      ...
      searchable by content. Runs off-thread with cancellation (the
      Insights discipline). Design question first: compose with the
      search grammar or a standalone pre-filter.
- [ ] **The S-rider bundle** (four coherence wins, one release):
      Reload Library menu item (`refresh_library()` is written and
      uncallable), clickable Insights audit rows (selection-mode NONE
      dead-ends the full sample lists), avg-rating on genre pills,
      enum `label_index` ordering for custom-column pills.
- [ ] **Author browser** — genres and series have browse pages; author,
      the primary fiction dimension, has none. The SeriesBrowser card
      pattern ports directly.
- [ ] **Export respects the current view** — export always dumps the
      full library even under a VL/search filter (the subtitle shows
      "N of M"); ~20 lines reusing export_books verbatim.
- [ ] **Recently Added shelf row** — the sentinel-row mechanism exists
      (Recently Read); a second sentinel ordered by timestamp.
- [ ] **Cover-size / grid-density preference** — COVER_WIDTH is fixed
      at 180; retires the stale Phase 5 "grid density" config claim.
      M-sized: the thumbnail cache is scale-tier-keyed.
- [ ] **Cache-management preference row** — size display + confirmed
      clear (rides the destructive-op discipline).
- [ ] **Random-discover action** — honest note: closest to
      manufacturing work; rider-slot filler only.

### New work noticed while executing the final audit

- [ ] **Export computes on a worker thread** (final-audit L2.8, not in
