...
The **locked-DB fallback** from `CalibreDB._open` is already ported into `database._connect()` (snapshot copy of the `.db` plus `-wal`/`-shm` siblings when Calibre holds the lock, one stderr note, atexit cleanup) and is covered by a test that takes a real `BEGIN EXCLUSIVE` lock. The author query also follows cquarry's ordered-subquery approach (`ORDER BY bal.id`, pipe-escaped commas restored). **Custom columns** (`db.get_custom_columns()` / `db.load_custom_column()`) landed in v0.18.0: `database.load_library()` bulk-loads each column's values onto `Book.custom` (keyed by label; multi-valued → list), the Codex "Details" section renders text/enumeration values as exact-match filter pills and other datatypes as plain lines, and `search.py` matches `#label:` queries. Both Calibre storage shapes are handled cquarry-style (normalized value+link tables and direct-storage columns like `date_read`). Phase 11 is now complete.
